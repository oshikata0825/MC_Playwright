...
test.describe('TC_032 Role Based Access Control - Global Tariffs (Classic) Tree Expansion', () => {
  test.describe.configure({ mode: 'parallel' });
  
  users.forEach((user: any) => {
    
    test(`Perform Global Tariffs (Classic) tree expansion test for ${user.id}`, async ({ page }) => {
      ...
      // Step 2: Access Global Tariffs (Classic) and Expand Tree Node
      await test.step('Access Global Tariffs (Classic) and Expand Tree Node', async () => {
        ...
        // Step 2b: Scroll and Expand a Random Node
        // Scroll down by an arbitrary amount (e.g., 300px)
        try {
            console.log(`Debug: mainFrame is of type ${mainFrame.constructor.name}`);
            await mainFrame.locator('body').evaluate((body) => {
                body.ownerDocument.defaultView?.scrollBy(0, 300);
            });
        } catch (e) {
            console.warn('Scroll failed or mainFrame issue:', e);
        }
        await page.waitForTimeout(1000); // Wait for scroll

        // Find expand buttons (.rtPlus)
        // We use .rtPlus which typically indicates a collapsed node in Telerik RadTreeView
        const expandButtons = mainFrame.locator('.rtPlus');
        const count = await expandButtons.count();
        console.log(`Found ${count} expand buttons.`);

        if (count > 0) {
            // Select a random expand button
            const randomIndex = Math.floor(Math.random() * count);
            const targetButton = expandButtons.nth(randomIndex);
            
            // Ensure it is in view (though we already scrolled, this ensures Playwright can click it)
            await targetButton.scrollIntoViewIfNeeded();
            await page.waitForTimeout(500);

            console.log(`Clicking expand button at index ${randomIndex}.`);
            await targetButton.click();

            // Wait for expansion (content load or icon change)
            await page.waitForTimeout(3000);
            console.log('SUCCESS: Clicked expand button.');
        } else {
            console.warn('WARNING: No expand buttons (.rtPlus) found. Tree might be fully expanded or empty.');
        }

        // Take Screenshot
        const screenshotBufferResult = await page.screenshot({ fullPage: true });
        await test.info().attach(`global_tariffs_classic_tree_expansion_${user.id}.png`, {
