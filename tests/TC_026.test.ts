...
test.describe('TC_026 Role Based Access Control - Global Tariffs (Quick Lookup) Page Access', () => {
    ...
    test(`Perform Global Tariffs (Quick Lookup) page access test for ${user.id}`, async ({ page }) => {
      ...
      await test.step('Access Global Tariffs (Quick Lookup) and Verify', async () => {
        ...
        for (let i = 1; i <= 5; i++) {
            ...
            await hsInput.pressSequentially(criteria.hsCode, { delay: 150 });
            ...
            await test.info().attach(`global_tariffs_quick_lookup_result_${user.id}.png`, {
