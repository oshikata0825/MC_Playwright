...
test.describe('TC_019 Role Based Access Control - Regulation List Updates Sort', () => {
    ...
    test(`Perform regulation list updates sort test for ${user.id}`, async ({ page }) => {
      ...
      await test.step('Perform Search 5 times or until screenshot is taken', async () => {
        ...
        for (let i = 1; i <= 5; i++) {
            ...
            // Click Last Source Update Date column header to sort
            console.log('INFO: Clicking "Last Source Update Date" column header to sort...');
            // Based on elements.txt, the element is an <a> tag with title "Click here to sort" and no ID attribute.
            const sortHeader = mainFrame.locator('a[title="Click here to sort"]').filter({ hasText: 'Last Source Update Date' });
            await expect(sortHeader).toBeVisible({ timeout: 10000 });
            
            // First click (usually Ascending)
            await sortHeader.click();
            await page.waitForTimeout(3000); // Wait for grid update

            // Second click (usually Descending - Newest First)
            console.log('INFO: Clicking again to set Descending order (Newest First)...');
            await sortHeader.click();
            ...
            await test.info().attach(`regulation_list_updates_sorted_${user.id}.png`, {
                body: screenshotBuffer,
                contentType: 'image/png'
            });

            console.log('SUCCESS: Sorted and screenshot attached. Test complete.');
            break;
        }
      });
